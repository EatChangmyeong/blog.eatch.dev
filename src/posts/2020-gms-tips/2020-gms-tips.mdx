...
function debugged(_value) {
	show_debug_message(_value);
	return _value;
}

// 우변이 실행되지 않습니다.
if(debugged(1) || debugged(0)) {}
// 1

// 우변이 실행되지 않습니다.
if(debugged(0) && debugged(1)) {}
// 0
```
...
현재의 GMS2에서는 <Term term="handle">핸들</Term>이라는 자료형으로 에셋뿐만 아니라 자료구조, 인스턴스, 스크립트 함수, 파티클 시스템, 버퍼, 서피스 등 여러 가지 자원을 관리하고 있고, `typeof` 함수에 전달하면 `"ref"`를 반환합니다. 함수는 다소 복잡한데, 스크립트에서 선언한 함수는 핸들, 오브젝트에서 선언한 함수는 수, 메소드는 별도의 `"method"` 자료형으로 관리되는 듯합니다.
...
* `"bool"`: 기존 GameMaker에서는 `true`가 *실수* 1, `false`가 *실수* 0으로 정의되었지만 현재는 **별도의** 자료형으로 구분되었고 `1`과 `true`의 구분이 가능합니다.
...
충격적이게도 0.1 + 0.2는 0.3이 **아닙니다**. GameMaker처럼 배정밀도 부동소숫점을 기본으로 하는 JavaScript로 확인해 보면 이렇습니다.
...
export const footnotes = {
	...
	'short-circuit-evaluation': '\'단락\'은 전기회로의 단락에서 따온 것이고, \'평가\'는 식의 값을 구하는 것을 유식하게 이르는 말이라고 생각하면 됩니다.',
