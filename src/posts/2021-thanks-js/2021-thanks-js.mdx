...
<Blockquote>
	*Number 타입*은 정확히 18,437,736,874,454,810,627(즉, $$2^{64} - 2^{53} + 3$$)개의 값을 가지고, 배정밀도 부동소수점의 <ADoi doi="10.1109/IEEESTD.2019.8766229" restricted>IEEE 754-2019</ADoi> binary64 값을 IEEE 이진 부동소수점 산술 표준에서 명시한 대로 나타내나, IEEE 표준의 9,007,199,254,740,990(즉, $$2^{53} - 2$$)개의 서로 다른 NaN 값은 ECMAScript에서 특수한 단일 **NaN** 값으로 나타내어진다. (**NaN** 값은 프로그램 표현식 `NaN`으로 생성됨에 유의하라.) ...
...
다만, `{}+[]`는 예외적으로 객체 `{}`와 배열 `[]`을 더하는 것이 아니라 **비어 있는 블록** `{}`과 배열에 단항 덧셈을 하는 표현식 `+[]`으로 파싱되기 때문에 다른 결과가 나옵니다. 이 값은 <AOut href={h.abs.conv.tonumber.$}>ToNumber</AOut>(<Var>argument</Var> = **[]**), <AOut href={h.abs.conv.toprimitive}>ToPrimitive</AOut>(<Var>input</Var> = **[]**, <Var>preferredType</Var> = NUMBER)를 거쳐 <strong>""</strong>이 된 뒤 <AOut href={h.abs.conv.tonumber.$}>ToNumber</AOut>(<Var>argument</Var> = **""**), <AOut href={h.abs.conv.tonumber.string.stringtonumber}>StringToNumber</AOut>(<Var>str</Var> = **""**), 마지막으로 <AOut href={h.abs.conv.tonumber.string.stringnumericvalue}>StringNumericValue</AOut>(*StringNumericLiteral* ::: <Var>StrWhiteSpace</Var><sub>opt</sub>)의 많은 단계를 거쳐 $$+0_\mathbb{F}$$, 즉 `0`이 됩니다. `+[]` = `0`이라는 내용을 이렇게까지 늘여서 써도 되나...? 참고로 `({}+[])`은 정상적(?)으로 `"[object Object]"`가 됩니다.
...
export const footnotes = {
	'ecma262-language-types': () => <>ECMA-262 표준에서 정의하는 "<Term term="language types"><AOut href={h.type.lang.$}>언어 타입</AOut></Term>"은 <code>typeof</code>가 반환하는, '사용자에게 노출되는' 타입과 다소 차이가 있는데, <code>null</code>은 Object가 아닌 별도의 Null 타입으로, <code>typeof</code>가 <code>"function"</code>인 값은 내부 메소드 [[Call]]이 있는 Object 타입으로 분류되어 있습니다.</>,
	'numeric-separator': () => <>ES2021부터 수를 작성할 때는 숫자 사이에 <code>_</code> 문자를 삽입해서 자릿수를 구분할 수 있으며, 소수점 앞뒤, 지수 자리, 이진수, 팔진수, 십육진수, BigInt를 모두 지원합니다.</>,
	'max-identity': () => <>엄밀히 말해 max의 항등원은 <AWp art="Extended real number line">확장된 실수</AWp>, 즉 ±∞가 추가된 실수 체계에서만 정의되며, 일반적인 실수 집합에서는 max의 항등원이 존재하지 않습니다. 추가로 JavaScript의 Number에는 <code>NaN</code>이 정의되어 있는데, max(NaN, -∞) = max(-∞, NaN) = NaN이므로 문제가 없습니다.</>,
	supremum: () => <>물론 더 엄밀하게 따지면 최댓값과 최솟값은 <em>주어진 집합의 원소 중에서</em> 골라야 하는 값이므로 공집합에 대해서 정의되지 않기 때문에, 이렇게 정의할 거면 이름을 <AWp lang="ko" art="상한과 하한"><code>supremum</code>/<code>infimum</code></AWp>으로 지어야 한다는 반박이 가능합니다.</>,
	tostring: () => <>ToString은 String이 아닌 값을 String인 값으로 바꾸는 추상 연산입니다. 이 맥락에서는 <Var>lPrim</Var>과 <Var>rPrim</Var>이 모두 String이므로 의미 있는 동작을 하지 않습니다.</>,
};

export const transnotes = {
	significand: () => <><AOut href="https://mathbang.net/599">상용로그의 지표와 가수 개념</AOut>에서 번역어를 차용했습니다.</>
};

export const references = {
	ECMA262: () => <>ECMA-262 <AOut href="https://tc39.es/ecma262/multipage/"><em>ECMAScript® 2026 Language Specification</em></AOut>. 2025년 8월 21일 현재 2026년판으로, 판올림 시 목차 번호나 세부 알고리즘이 바뀔 수 있지만 글의 취지에는 영향이 없습니다.</>,
};