...
<Blockquote>
	**6.7.4.2 `restrict`의 형식적 정의**
	...
	<sup>139&#41;</sup> 즉, <var>E</var>가 <var>P</var>를 통해 간접적으로 참조되는 개체의 값이 아니라 <var>P</var>의 값에 직접 의존하는 경우를 말한다. 예를 들어, 식별자 <var>p</var>가 `(int **restrict)` 타입을 가질 경우, 포인터 표현식 `p`와 `p+1`은 <var>p</var>가 지시하는 restrict 한정된 포인터 개체에 기반하지만, 포인터 표현식 `*p`와 `p[1]`은 그렇지 않다.
